function [affs,rects,points, scales, rotations, time] = tracker(video_path, img_files, pos, target_sz, rot, show_visualization)
	...
	for frame = 1:numel(img_files),
       ...
       %% construct return variables
        ...
        aff = [];
        if p.isRotation
            T = parameters_to_projective_matrix('SIMILARITY', [1,p.rot,pos(2),pos(1)]);
            [aff,~]= getLKcorner(T, target_sz);
            aff(:,5)=aff(:,1);        
            points(frame,:) = [aff(1,1),aff(2,1),aff(1,4),aff(2,4),aff(1,3),aff(2,3),aff(1,2),aff(2,2)];
